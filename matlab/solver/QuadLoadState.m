classdef QuadLoadState < DifferentialFlatness
	...
	methods
		...
		function flatOutputsToStates(obj, xL, vL, aL, daL, d2aL, d3aL, d4aL)
			g = [0;0;-9.8];
			err = 0.01;
			if norm(aL - g) >= err
				...
				% todo
				[obj.xL,obj.vL,~,obj.q,~,~,obj.R,obj.omega,~,obj.Omega,~,~,~,~,~,~,~,~] = obj.getNomTrajSlack(xL,vL,aL,daL,d2aL,d3aL,d4aL);
				obj.status = false;
			end
		end
